...
- **AI:** Cloudflare Workers AI dịch và tóm tắt tin quốc tế bằng binding trực tiếp, không cần API key; OpenAI và mock vẫn là fallback tùy chọn.
...
| `AI_PROVIDER`                                 | `mock`, `cloudflare`, `openai`  | Không, mặc định mock  |
| `CLOUDFLARE_AI_MODEL`                         | Model của Workers AI            | Không                 |
...
- Tin quốc tế được dịch và tóm tắt sang tiếng Việt qua Cloudflare Workers AI khi cấu hình `AI_PROVIDER=cloudflare` và binding `AI`; không cần API key. Nếu AI lỗi hoặc hết hạn mức, bài gốc tiếng Anh vẫn hiển thị minh bạch.

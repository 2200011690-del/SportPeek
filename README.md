...
- **Ingestion:** `NewsProvider` cho Mock/RSS/JSON, timeout, retry giới hạn, chuẩn hóa, content hash và heuristic phát hiện trùng.
- **Thể thao:** `SportsDataProvider` với mock implementation; dễ thay thế bằng nhà cung cấp live data.
...
## Provider đang dùng mock

- AI classify/summary/entity/preview/recap.
- News ingestion mặc định (RSS và JSON adapter đã sẵn sàng).
- Live score, fixtures, results, standings, team và player data.

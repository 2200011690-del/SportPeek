...
type MatchRow = {
  ...
  updated_at: string;
  competitions: Joined<NamedRow>;
  home_team: Joined<NamedRow>;
  away_team: Joined<NamedRow>;
};
type StandingRow = {
  position: number;
  played: number;
...
const freshness = (value: string): "fresh" | "delayed" | "stale" | "unknown" =>
  value === "fresh" || value === "delayed" || value === "stale"
    ? value
    : "unknown";
...
const mapCompetition = (row: CompetitionRow): Competition => ({
  id: row.id,
  name: row.name,
  slug: row.slug,
  country: row.country ?? "",
  season: row.current_season ?? "Chưa xác định",
  logoUrl: row.logo_url ?? undefined,
});
...
function mapMatch(row: MatchRow): Match {
  ...
  return {
    id: row.id,
    competition: competition?.name ?? "Chưa xác định",
    home: home?.name ?? "Chưa xác định",
    away: away?.name ?? "Chưa xác định",
    homeScore:
    ...
    startTimestamp: row.start_time,
    status,
    minute: row.minute ?? undefined,
    venue: row.venue ?? "Chưa công bố",
...
function mapStanding(row: StandingRow, stale = false): Standing {
  return {
    ...
    team: one(row.teams)?.name ?? "Chưa xác định",
    played: row.played,
    won: row.won,
    drawn: row.drawn,
    lost: row.lost,
    ...
    points: row.points,
    form: (row.form ?? []).filter((value): value is "W" | "D" | "L" =>
      ["W", "D", "L"].includes(value),
    ),
    provider: row.provider ?? "unknown",
    sourceTimestamp: row.source_timestamp ?? undefined,
    dataFreshness: stale ? "stale" : freshness(row.data_freshness),
    competitionId: row.competition_id,
    competition: one(row.competitions ?? null)?.name,
    season: row.season,
  };
}

export type CachedSportsRead<T> = {
  data: T[];
...
export class SportsCacheRepository {
  private client() {
    ...
    if (!client)
      throw new ConfigurationError(
        "Supabase service role chưa được cấu hình cho sports cache.",
        "supabase",
      );
    return client;
  }
  ...
  async readMatches(
    ...
    const stale = Boolean(
      ...
      Date.now() - Date.parse(updatedAt) >
        (kind === "live" ? 5 * 60_000 : 6 * 60 * 60_000),
    );
    return {
      data: rows.map((row) => ({
        ...mapMatch(row),
        dataFreshness: stale ? "stale" : mapMatch(row).dataFreshness,
      })),
      updatedAt,
      stale,
      provider:
        [...new Set(rows.map((row) => row.provider).filter(Boolean))].join(
          ", ",
        ) || "cache",
      source: "supabase",
    };
  ...
  async readStandings(): Promise<CachedSportsRead<Standing>> {
    const { data, error } = await this.client()
      .from("standings")
      .select(
        "competition_id,season,position,played,won,drawn,lost,goals_for,goals_against,points,form,provider,source_timestamp,data_freshness,updated_at,competitions(id,name,slug),teams(id,name,slug)",
      )
      .order("position")
      .limit(500);
    if (error)
      throw new ProviderError("Không thể đọc bảng xếp hạng cache.", "supabase");
    const rows = (data ?? []) as unknown as StandingRow[];
    const updatedAt = maxTimestamp(rows.map((row) => row.updated_at));
    const stale = Boolean(
      updatedAt && Date.now() - Date.parse(updatedAt) > 24 * 60 * 60_000,
    );
    return {
      data: rows.map((row) => mapStanding(row, stale)),
      updatedAt,
      stale,
      provider:
        [...new Set(rows.map((row) => row.provider).filter(Boolean))].join(
  ...
  async readTeams(): Promise<CachedSportsRead<Team>> {
    ...
    return {
      ...
      updatedAt,
      stale: Boolean(
        updatedAt && Date.now() - Date.parse(updatedAt) > 7 * 24 * 60 * 60_000,
      ),
      provider: "cache",
      source: "supabase",
    };
  ...
  async readCompetitions(): Promise<CachedSportsRead<Competition>> {
    ...
    const [competitionsResult, linksResult] = await Promise.all([
      client
        .from("competitions")
        .select("id,name,slug,country,current_season,logo_url,updated_at")
        .eq("is_active", true)
        .order("name")
        .limit(200),
      client.from("competition_teams").select("competition_id").limit(2_000),
    ]);
    if (competitionsResult.error || linksResult.error)
      throw new ProviderError(
        "Không thể đọc danh sách giải cache.",
        "supabase",
      );
    const populated = new Set(
      (linksResult.data ?? []).map((row) => row.competition_id),
    );
    const rows = ((competitionsResult.data ?? []) as CompetitionRow[]).filter(
      (row) => populated.has(row.id),
    );
    const updatedAt = maxTimestamp(rows.map((row) => row.updated_at));
    return {
      data: rows.map(mapCompetition),
      updatedAt,
      stale: Boolean(
        updatedAt && Date.now() - Date.parse(updatedAt) > 30 * 24 * 60 * 60_000,
      ),
      provider: "cache",
      source: "supabase",
    };
  ...
  async readMatch(id: string): Promise<MatchDetailData | null> {
    ...
    const { data, error } = await client
      ...
      .maybeSingle();
    if (error)
      throw new ProviderError("Không thể đọc chi tiết trận đấu.", "supabase");
    if (!data) return null;
    ...
    const [eventsResult, statisticsResult, standingsResult, coverageResult] =
      await Promise.all([
        client
          .from("match_events")
          .select(
            "id,event_type,minute,extra_minute,teams(name),player:players!match_events_player_id_fkey(name),related_player:players!match_events_related_player_id_fkey(name)",
          )
          .eq("match_id", id)
          .order("minute"),
        client
        ...
        client
          .from("standings")
          .select(
            "position,played,won,drawn,lost,goals_for,goals_against,points,form,provider,source_timestamp,data_freshness,updated_at,teams(id,name,slug)",
          )
        ...
        client
          ...
          .select("capability,primary_provider")
          .eq("competition_id", row.competition_id)
          .eq("active", true),
      ]);
    const readError =
    ...
    if (readError)
      throw new ProviderError(
        "Không thể đọc các phần dữ liệu trận đấu.",
        "supabase",
      );
    const events = (
      (eventsResult.data ?? []) as unknown as Array<{
        id: string;
        event_type: string;
        minute: number;
        extra_minute: number | null;
        teams: Joined<NamedRow>;
        player: Joined<NamedRow>;
        related_player: Joined<NamedRow>;
      }>
      ...
      team: one(event.teams)?.name,
      player: one(event.player)?.name,
      relatedPlayer: one(event.related_player)?.name,
    }));
    const statistics = (
      (statisticsResult.data ?? []) as unknown as Array<{
    ...
    ).map<MatchStatistic>((stat) => ({
      team: one(stat.teams)?.name ?? "Chưa xác định",
      possession: stat.possession ?? undefined,
      shots: stat.shots ?? undefined,
      shotsOnTarget: stat.shots_on_target ?? undefined,
      corners: stat.corners ?? undefined,
      fouls: stat.fouls ?? undefined,
      yellowCards: stat.yellow_cards ?? undefined,
    ...
    const capabilities: Record<MatchCapability, boolean> =
      deriveMatchCapabilities({
        status: base.status,
        venue: row.venue,
        referee: row.referee,
    ...
    const stale =
      ...
      (base.status === "live" ? 5 * 60_000 : 6 * 60 * 60_000);
    return {
      match: {
        ...base,
        competitionId: row.competition_id,
        ...
        dataFreshness: stale ? "stale" : base.dataFreshness,
      },
      events,
      statistics,
  ...
  async readCompetition(slug: string): Promise<CompetitionDetailData | null> {
    ...
    const [
      ...
      coverageResult,
        ...
        client
          ...
          .select(
            "teams(id,name,short_name,slug,country,stadium,logo_url,updated_at)",
          )
    ...
    const readError =
      ...
      coverageResult.error;
    if (readError)
      throw new ProviderError("Không thể đọc dữ liệu giải đấu.", "supabase");
    const teams = (
      (teamsResult.data ?? []) as unknown as Array<{ teams: Joined<TeamRow> }>
    ).flatMap((item) => {
    ...
    const standings = (
      (standingsResult.data ?? []) as unknown as StandingRow[]
    ).map((row) => mapStanding(row));
    return {
  ...
  async readTeam(slug: string): Promise<TeamDetailData | null> {
    ...
    const [
      ...
      standingsResult,
        ...
        client
          .from("competition_teams")
          .select(
            "competitions(id,name,slug,country,current_season,logo_url,updated_at)",
          )
          .eq("team_id", team.id),
        client
          .from("matches")
    ...
    const competitions = (
      (competitionsResult.data ?? []) as unknown as Array<{
        competitions: Joined<CompetitionRow>;
      }>
    ).flatMap((item) => {
  ...
  async readPlayer(slug: string): Promise<PlayerDetailData | null> {
    ...
    const { data, error } = await client
      ...
      .maybeSingle();
    if (error)
      throw new ProviderError("Không thể đọc hồ sơ cầu thủ.", "supabase");
    if (!data) return null;

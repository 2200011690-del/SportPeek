...
export async function providerFetch(
  ...
  url: string,
  init: RequestInit = {},
  options: { timeoutMs?: number; retries?: number; minimumIntervalMs?: number } = {},
): Promise<Response> {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      if ([401, 403].includes(response.status)) {
        throw new ProviderError(`${provider} từ chối API key.`, provider, false);
      }
      if (response.status === 429 && attempt === retries) {
        throw new RateLimitError(`${provider} đã hết quota tạm thời.`);
      }
      if (
      ...
      if (attempt === retries) {
        throw new ProviderError(
          `${provider} không phản hồi trong thời hạn.`,
          provider,
          true,
        );
      }

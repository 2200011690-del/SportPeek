...
async function fetchWithRetry(url: string, attempts = 2): Promise<Response> {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try { const response = await fetch(url, { signal: AbortSignal.timeout(8_000), headers: { "user-agent": "NewsPeek/1.0 (+https://sportpeek.2200011690.workers.dev/sources)" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return response; } catch (caught) { error = caught; }
  }

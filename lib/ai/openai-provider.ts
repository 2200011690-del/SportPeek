...
export class OpenAIProvider extends RemoteAIProvider {
  ...
  protected async structured<T>(schema: z.ZodType<T>, task: string, input: unknown): Promise<T> {
    ...
    const response = await providerFetch(this.name, "https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${key}`, "content-type": "application/json" }, body: JSON.stringify({ model: process.env.OPENAI_MODEL || "gpt-5.4-nano", input: [{ role: "system", content: `Bạn là biên tập viên tin tức trung lập của NewsPeek. ${task} Trả JSON thuần, không markdown.` }, { role: "user", content: JSON.stringify(input) }], text: { format: { type: "json_schema", name: "newspeek_structured_output", strict: true, schema: z.toJSONSchema(schema) } }, max_output_tokens: 4000 }) }, { timeoutMs: 35_000, retries: 1, minimumIntervalMs: 250 });

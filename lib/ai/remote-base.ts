...
export const summarySchema = z.object({ title: z.string().min(1).max(500), summary: z.string().min(80).max(1800), keyPoints: z.array(z.string().min(1)).min(1).max(5), sourceIds: z.array(z.string()) });
...
export abstract class RemoteAIProvider implements AIProvider {
  ...
  summarizeCluster(input: { articles: ClusterArticleInput[] }) { return this.structured(summarySchema, "Tóm tắt cụm tin bằng tiếng Việt trong 80–180 từ. Không thêm tỷ số, thống kê, phát biểu, phí chuyển nhượng hay chấn thương ngoài nguồn. sourceIds chỉ chứa ID đầu vào.", input); }

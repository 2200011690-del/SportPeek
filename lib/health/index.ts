...
export async function getHealthSnapshot(): Promise<HealthSnapshot> {
  ...
  const [sources, rssJob, storyJob, clusters, sportsSync, matches, standings] = await Promise.all([
    ...
    client.from("story_clusters").select("id", { count: "exact", head: true }),
  ...
  const storyUpdated = storyJob.data?.completed_at ?? null; let storyState = ageState(storyUpdated, 2 * 60 * 60_000); if (storyJob.error || clusters.error) storyState = "unavailable"; else if (storyJob.data?.status === "failed") storyState = "degraded";
  ...
  const ai = getAIProvider(); const aiState: HealthState = ai.name === "mock" ? "development_mock" : ai.name === "heuristic" || ai.name === "disabled" ? "configuration_required" : "operational";
  ...
  const services = {
    ...
    stories: service(storyState, storyState === "operational" ? `Stories · ${clusters.count ?? 0} cụm` : "Story pipeline cần kiểm tra", "Feed chỉ đọc cluster đã xử lý trong Supabase.", "supabase", storyUpdated, clusters.count ?? 0),
    ...
    ai: service(aiState, aiState === "operational" ? `AI · ${ai.name}` : aiState === "development_mock" ? "AI · fixture phát triển" : "AI remote chưa bật", aiState === "configuration_required" ? "Story vẫn đọc được bằng summary heuristic; không giả là bản dịch AI." : "AI chỉ chạy trong job xử lý.", ai.name, null, null),

...
type RuntimeData = { newsItems: NewsItem[]; matchItems: Match[]; standingRows: typeof standings; newsReal: boolean; sportsReal: boolean };
type RuntimeResponse<T> = { data: T; demo?: boolean; provider?: string };
const RuntimeDataContext = createContext<RuntimeData>({ newsItems: news, matchItems: matches, standingRows: standings, newsReal: false, sportsReal: false });
...
function NewsPage({ bookmarks, onBookmark }: { bookmarks: Set<string>; onBookmark: (id: string) => void }) {
  ...
  return <div className="page-content"><PageHero eyebrow="NEWSROOM" title="Tin mới nhất" description="Tin tức từ nguồn chính thức được chuẩn hóa, tóm tắt và đánh giá tự động."><div className="hero-stat"><strong>{newsItems.length}</strong><span>{newsReal ? "tin chính thức" : "tin dự phòng"}</span></div></PageHero><FilterBar search /><div className="news-page-grid">{newsItems.map((item) => <NewsCard key={item.id} item={item} bookmarked={bookmarks.has(item.id)} onBookmark={onBookmark} />)}</div><Pagination /></div>;
}
...
function LivePage({ mode }: { mode: "live" | "fixtures" | "results" }) {
  ...
  return <div className="page-content"><PageHero eyebrow={labels[0]} title={labels[1]} description={sportsReal ? "Dữ liệu được cập nhật trực tiếp từ API-Football." : labels[2]}>{mode === "live" && <div className="live-count"><i />{filtered.filter((item)=>item.status==="live").length} trận đang diễn ra</div>}</PageHero>{mode !== "live" && <div className="date-nav"><button><ChevronLeft size={18} /></button><button className="active">Hôm nay<small>{new Intl.DateTimeFormat("vi-VN",{day:"2-digit",month:"2-digit"}).format(new Date())}</small></button><button>Ngày mai</button><button><ChevronRight size={18} /></button><button className="calendar-button"><CalendarDays size={17} />Chọn ngày</button></div>}<FilterBar /><div className="match-groups">{[...new Set(filtered.map((item)=>item.competition))].map((competitionName) => { const group = filtered.filter((item) => item.competition === competitionName); return <section className="match-group" key={competitionName}><div className="competition-title"><span className="competition-icon">SP</span><div><strong>{competitionName}</strong><span>{sportsReal ? "Dữ liệu trực tiếp" : "Dữ liệu dự phòng"}</span></div><ChevronRight size={18} /></div><div className="match-grid">{group.map((match) => <MatchCard key={match.id} match={match} />)}</div></section>; })}</div></div>;
}

function StandingsPage() {
  const { sportsReal } = useRuntimeData();
  return <div className="page-content"><PageHero eyebrow="MÙA GIẢI HIỆN TẠI" title="Bảng xếp hạng" description={sportsReal ? "Thứ hạng và phong độ được cập nhật trực tiếp từ API-Football." : "Đang hiển thị dữ liệu dự phòng cho tới khi API thể thao được kích hoạt."}><div className="season-select"><Trophy size={18} /><span>Premier League</span><ChevronDown size={16} /></div></PageHero><div className="standings-panel"><div className="panel-tabs"><button className="active">Bảng tổng</button><button>Sân nhà</button><button>Sân khách</button><span>{sportsReal ? "Nguồn API-Football" : "Dữ liệu dự phòng"}</span></div><StandingsTable full /><div className="table-legend"><span><i className="champions" />Champions League</span><span><i className="europa" />Europa League</span><span><i className="relegation" />Xuống hạng</span></div></div></div>;
}
...
function NewsDetail({ slug, bookmarked, onBookmark }: { slug: string; bookmarked: boolean; onBookmark: (id: string) => void }) {
  ...
  return <div className="article-page"><div className="article-breadcrumb"><Link href="/news">Tin tức</Link><ChevronRight size={14} /><span>{item.competition}</span></div><header className="article-header"><div className="article-badges"><span className="demo-label">{item.originalUrl ? "NGUỒN CHÍNH THỨC" : "DỮ LIỆU MINH HỌA"}</span><HotnessBadge score={item.hotness} /><ReliabilityBadge score={item.reliability} /></div><h1>{item.title}</h1><p>{item.summary}</p><div className="article-meta"><span className="source-avatar">SP</span><div><strong>SportPeek AI Desk</strong><span>Cập nhật {item.publishedAt} · Đọc 4 phút</span></div><div className="article-actions"><button onClick={() => onBookmark(item.id)} className={bookmarked ? "active" : ""}><Bookmark size={17} fill={bookmarked ? "currentColor" : "none"} />{bookmarked ? "Đã lưu" : "Lưu"}</button><button><Share2 size={17} />Chia sẻ</button></div></div></header><NewsVisual item={item} /><div className="article-layout"><article className="article-body"><div className="summary-box"><div className="summary-title"><Sparkles size={19} /><strong>Tóm tắt bởi AI</strong><span>80–150 từ</span></div><p>{item.summary} Nội dung được tổng hợp trung lập từ metadata và trích đoạn ngắn. SportPeek không sao chép toàn văn và không bổ sung dữ kiện chưa xuất hiện trong nguồn.</p></div><section><h2>Những điểm chính</h2><ul className="key-points">{item.keyPoints.map((point) => <li key={point}><Check size={16} />{point}</li>)}</ul></section><section><h2>Dòng thời gian cập nhật</h2><div className="timeline"><div><time>Mới</time><span><i /><strong>Cụm tin được cập nhật</strong><p>Nguồn chính thức được kiểm tra và điểm tin cậy được tính lại.</p></span></div><div><time>AI</time><span><i /><strong>Tóm tắt hoàn tất</strong><p>Hệ thống tạo tiêu đề chuẩn hóa và các ý chính.</p></span></div><div><time>RSS</time><span><i /><strong>Phát hiện bài viết</strong><p>Metadata được chuẩn hóa và kiểm tra trùng lặp.</p></span></div></div></section><div className="aggregation-notice"><ShieldCheck size={22} /><div><strong>Đây là nội dung tổng hợp</strong><p>SportPeek chỉ hiển thị metadata, trích đoạn ngắn và bản tóm tắt. Hãy truy cập nguồn gốc để đọc nội dung đầy đủ.</p></div></div></article><aside className="article-aside"><div className="rail-card"><SectionHeading eyebrow="ĐƯỢC SỬ DỤNG" title={`${item.sources.length} nguồn`} />{item.sources.map((source, index) => <a href={item.originalUrl ?? "https://example.com"} target="_blank" rel="noreferrer" className="source-card" key={source}><span className="source-avatar">{index + 1}</span><div><strong>{source}</strong><span>{item.originalUrl ? "Nguồn chính thức" : "Nguồn minh họa"} · Tin cậy {item.reliability - index * 4}%</span></div><ExternalLink size={15} /></a>)}</div><div className="rail-card"><SectionHeading eyebrow="LIÊN QUAN" title="Thực thể" /><div className="entity-chips"><Link href="/news"><Newspaper size={15} />Bóng đá Việt Nam</Link><Link href="/competitions/v-league-1"><Trophy size={15} />{item.competition}</Link></div></div></aside></div><section className="related-news"><SectionHeading eyebrow="ĐỌC TIẾP" title="Tin liên quan" /><div className="news-page-grid">{newsItems.filter((entry) => entry.id !== item.id).slice(0, 3).map((entry) => <NewsCard key={entry.id} item={entry} bookmarked={false} onBookmark={onBookmark} />)}</div></section></div>;
}
...
export default function SportPeekApp({ route }: { route: string }) {
  ...
  const [runtimeData, setRuntimeData] = useState<RuntimeData>({ newsItems: news, matchItems: matches, standingRows: standings, newsReal: false, sportsReal: false });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setRuntimeData({ newsItems: newsResponse?.data?.length ? newsResponse.data : news, matchItems: mergedMatches.length ? mergedMatches : matches, standingRows: tableResponse?.data?.length ? tableResponse.data : standings, newsReal: newsResponse?.demo === false, sportsReal: sportsResponses.some((result) => result.provider === "api-football" && result.demo === false) });
    };
    void load();
    return () => { active = false; };
  }, []);
  ...
  return <RuntimeDataContext.Provider value={runtimeData}><div className="app-shell"><AppSidebar route={route} open={menuOpen} onClose={() => setMenuOpen(false)} /><div className="app-column"><Header onMenu={() => setMenuOpen(true)} onSearch={() => setSearchOpen(true)} theme={theme} onTheme={() => setTheme(theme === "dark" ? "light" : "dark")} /><div className="demo-bar"><ShieldCheck size={14} />{runtimeData.newsReal ? "Tin tức đang lấy trực tiếp từ VFF và VPF." : "Nguồn tin chính thức tạm gián đoạn — đang dùng dữ liệu dự phòng."} {runtimeData.sportsReal ? "Tỉ số đang cập nhật từ API-Football." : "Tỉ số sẽ chuyển sang trực tiếp sau khi kích hoạt API key."}</div><div className="content-wrap">{page}</div><AppFooter /></div><MobileNavigation route={route} /><SearchCommand open={searchOpen} onClose={() => setSearchOpen(false)} /></div></RuntimeDataContext.Provider>;
}

...
function NewsVisual({ item, compact = false }: { item: NewsItem; compact?: boolean }) {
  return <div className={`news-visual tone-${item.imageTone} ${compact ? "compact" : ""}`}>
    <div className="field-lines" /><span className="visual-label">SPORTPEEK / DEMO</span><span className="visual-team">{getInitials(item.team)}</span>
  </div>;
}
...
function HomePage({ bookmarks, onBookmark }: { bookmarks: Set<string>; onBookmark: (id: string) => void }) {
  return <><BreakingTicker /><div className="home-grid"><main className="main-feed"><div className="welcome-row"><div><span className="eyebrow">THỨ HAI · 13 THÁNG 7</span><h1>Chào buổi tối, người hâm mộ.</h1><p>Những diễn biến đáng chú ý được tổng hợp và kiểm chứng cho bạn.</p></div><div className="signal"><span><i />Hệ thống ổn định</span><strong>128</strong><small>tin đã phân tích hôm nay</small></div></div>
    <section><SectionHeading eyebrow="ĐIỂM TIN" title="Đáng chú ý nhất" action="Xem tất cả" /><div className="featured-grid">{news.slice(0, 2).map((item) => <NewsCard key={item.id} item={item} featured bookmarked={bookmarks.has(item.id)} onBookmark={onBookmark} />)}</div></section>
    <section><SectionHeading eyebrow="CẬP NHẬT LIÊN TỤC" title="Tin mới nhất" action="Mở bảng tin" /><div className="news-stack">{news.slice(2, 7).map((item) => <NewsListItem item={item} key={item.id} />)}</div></section>
  ...
  </main><aside className="right-rail"><section className="rail-card live-rail"><SectionHeading eyebrow="ĐANG DIỄN RA" title="Trực tiếp" action="Tất cả" href="/live" />{matches.filter((match) => match.status === "live").map((match) => <MatchCard key={match.id} match={match} compact />)}</section><section className="rail-card"><SectionHeading eyebrow="HÔM NAY" title="Lịch thi đấu" action="Lịch đầy đủ" href="/fixtures" />{matches.filter((match) => match.status === "scheduled").slice(0, 3).map((match) => <MatchCard key={match.id} match={match} compact />)}</section><section className="rail-card"><SectionHeading eyebrow="PREMIER LEAGUE" title="Bảng xếp hạng" action="Chi tiết" href="/standings" /><StandingsTable /></section><section className="rail-card topics"><SectionHeading eyebrow="XU HƯỚNG" title="Chủ đề nổi bật" /><div>{["# Kỳ chuyển nhượng", "# Đại chiến cuối tuần", "# Tài năng trẻ", "# Chiến thuật pressing", "# V.League"].map((topic, index) => <Link href={`/search?q=${encodeURIComponent(topic)}`} key={topic}><span>{topic}</span><em>{14 - index * 2} tin</em></Link>)}</div></section></aside></div></>;
}
...
function NewsPage({ bookmarks, onBookmark }: { bookmarks: Set<string>; onBookmark: (id: string) => void }) {
  return <div className="page-content"><PageHero eyebrow="NEWSROOM" title="Tin mới nhất" description="Tin tức từ nhiều nguồn được AI gom nhóm, tóm tắt và đánh giá tự động."><div className="hero-stat"><strong>128</strong><span>tin mới hôm nay</span></div></PageHero><FilterBar search /><div className="news-page-grid">{news.map((item) => <NewsCard key={item.id} item={item} bookmarked={bookmarks.has(item.id)} onBookmark={onBookmark} />)}</div><Pagination /></div>;
}
...
function LivePage({ mode }: { mode: "live" | "fixtures" | "results" }) {
  ...
  return <div className="page-content"><PageHero eyebrow={labels[0]} title={labels[1]} description={labels[2]}>{mode === "live" && <div className="live-count"><i />2 trận đang diễn ra</div>}</PageHero>{mode !== "live" && <div className="date-nav"><button><ChevronLeft size={18} /></button><button className="active">Hôm nay<small>13/07</small></button><button>Ngày mai<small>14/07</small></button><button>Thứ Tư<small>15/07</small></button><button><ChevronRight size={18} /></button><button className="calendar-button"><CalendarDays size={17} />Chọn ngày</button></div>}<FilterBar /><div className="match-groups">{competitions.slice(0, 4).map((competition) => { const group = filtered.filter((_, index) => index % 4 === competitions.indexOf(competition)); if (!group.length) return null; return <section className="match-group" key={competition.id}><div className="competition-title"><span className="competition-icon">{competition.country.slice(0, 2).toUpperCase()}</span><div><strong>{competition.name}</strong><span>{competition.country}</span></div><ChevronRight size={18} /></div><div className="match-grid">{group.map((match) => <MatchCard key={match.id} match={match} />)}</div></section>; })}</div></div>;
}

function StandingsPage() {
  return <div className="page-content"><PageHero eyebrow="MÙA GIẢI 2025/26" title="Bảng xếp hạng" description="Thứ hạng, phong độ và hiệu số được cập nhật từ provider dữ liệu minh họa."><div className="season-select"><Trophy size={18} /><span>Premier League</span><ChevronDown size={16} /></div></PageHero><div className="standings-panel"><div className="panel-tabs"><button className="active">Bảng tổng</button><button>Sân nhà</button><button>Sân khách</button><span>Cập nhật 5 phút trước</span></div><StandingsTable full /><div className="table-legend"><span><i className="champions" />Champions League</span><span><i className="europa" />Europa League</span><span><i className="relegation" />Xuống hạng</span></div></div></div>;
}
...
function NewsDetail({ slug, bookmarked, onBookmark }: { slug: string; bookmarked: boolean; onBookmark: (id: string) => void }) {
  ...
  return <div className="article-page"><div className="article-breadcrumb"><Link href="/news">Tin tức</Link><ChevronRight size={14} /><span>{item.competition}</span></div><header className="article-header"><div className="article-badges"><span className="demo-label">DỮ LIỆU MINH HỌA</span><HotnessBadge score={item.hotness} /><ReliabilityBadge score={item.reliability} /></div><h1>{item.title}</h1><p>{item.summary}</p><div className="article-meta"><span className="source-avatar">SP</span><div><strong>SportPeek AI Desk</strong><span>Cập nhật {item.publishedAt} · Đọc 4 phút</span></div><div className="article-actions"><button onClick={() => onBookmark(item.id)} className={bookmarked ? "active" : ""}><Bookmark size={17} fill={bookmarked ? "currentColor" : "none"} />{bookmarked ? "Đã lưu" : "Lưu"}</button><button><Share2 size={17} />Chia sẻ</button></div></div></header><NewsVisual item={item} /><div className="article-layout"><article className="article-body"><div className="summary-box"><div className="summary-title"><Sparkles size={19} /><strong>Tóm tắt bởi AI</strong><span>80–150 từ</span></div><p>{item.summary} Nội dung được tổng hợp trung lập từ metadata và trích đoạn ngắn của các nguồn minh họa. SportPeek không sao chép toàn văn và không bổ sung dữ kiện chưa xuất hiện trong nguồn.</p></div><section><h2>Những điểm chính</h2><ul className="key-points">{item.keyPoints.map((point) => <li key={point}><Check size={16} />{point}</li>)}</ul></section><section><h2>Dòng thời gian cập nhật</h2><div className="timeline"><div><time>20:42</time><span><i /><strong>Cụm tin được cập nhật</strong><p>Nguồn thứ hai được thêm và điểm tin cậy được tính lại.</p></span></div><div><time>20:18</time><span><i /><strong>Tóm tắt AI hoàn tất</strong><p>Hệ thống tạo tiêu đề chuẩn hóa và ba ý chính.</p></span></div><div><time>20:06</time><span><i /><strong>Phát hiện bài viết đầu tiên</strong><p>Bài gốc được chuẩn hóa và kiểm tra trùng lặp.</p></span></div></div></section><div className="aggregation-notice"><ShieldCheck size={22} /><div><strong>Đây là nội dung tổng hợp</strong><p>SportPeek chỉ hiển thị metadata, trích đoạn ngắn và bản tóm tắt do AI tạo. Hãy truy cập nguồn gốc để đọc nội dung đầy đủ.</p></div></div></article><aside className="article-aside"><div className="rail-card"><SectionHeading eyebrow="ĐƯỢC SỬ DỤNG" title={`${item.sources.length} nguồn`} />{item.sources.map((source, index) => <a href="https://example.com" target="_blank" rel="noreferrer" className="source-card" key={source}><span className="source-avatar">{index + 1}</span><div><strong>{source}</strong><span>Nguồn minh họa · Tin cậy {item.reliability - index * 4}%</span></div><ExternalLink size={15} /></a>)}</div><div className="rail-card"><SectionHeading eyebrow="LIÊN QUAN" title="Thực thể" /><div className="entity-chips"><Link href={`/teams/${teams.find((team) => team.name === item.team)?.slug ?? "arsenal"}`}><TeamMark name={item.team} size="sm" />{item.team}</Link><Link href={`/competitions/${competitions.find((c) => c.name === item.competition)?.slug ?? "premier-league"}`}><Trophy size={15} />{item.competition}</Link></div></div></aside></div><section className="related-news"><SectionHeading eyebrow="ĐỌC TIẾP" title="Tin liên quan" /><div className="news-page-grid">{news.filter((entry) => entry.id !== item.id).slice(0, 3).map((entry) => <NewsCard key={entry.id} item={entry} bookmarked={false} onBookmark={onBookmark} />)}</div></section></div>;
}

function MatchDetail({ id }: { id: string }) {
  const match = matches.find((item) => item.id === id) ?? matches[0];
  return <div className="page-content"><div className="match-detail-hero"><div className="match-detail-top"><span>{match.competition} · Dữ liệu minh họa</span><span className={match.status === "live" ? "live-pill" : "status-pill"}>{match.status === "live" ? `${match.minute}' · TRỰC TIẾP` : match.status === "finished" ? "ĐÃ KẾT THÚC" : "SẮP DIỄN RA"}</span></div><div className="scoreboard"><div><TeamMark name={match.home} size="lg" /><h2>{match.home}</h2><span>Chủ nhà</span></div><strong>{match.homeScore ?? 0}<em>–</em>{match.awayScore ?? 0}<small>{match.status === "scheduled" ? match.startTime : match.status === "live" ? `${match.minute}:24` : "FT"}</small></strong><div><TeamMark name={match.away} size="lg" /><h2>{match.away}</h2><span>Đội khách</span></div></div><div className="match-facts"><span><CalendarDays size={15} />13/07/2026 · {match.startTime}</span><span><MapPin size={15} />{match.venue}</span><span><UserRound size={15} />Trọng tài Demo</span></div></div><div className="panel-tabs match-tabs"><button className="active">Tổng quan</button><button>Đội hình</button><button>Thống kê</button><button>Đối đầu</button><button>Bảng xếp hạng</button></div><div className="match-detail-grid"><main><section className="content-card"><SectionHeading eyebrow="DIỄN BIẾN" title="Dòng thời gian" /><div className="event-timeline">{[["67'", "goal", match.home, "Bàn thắng · Minh Quân", "Kiến tạo: Alex Martin"], ["52'", "card", match.away, "Thẻ vàng · Marco Rossi", "Phạm lỗi chiến thuật"], ["38'", "goal", match.away, "Bàn thắng · Luka Silva", "Kiến tạo: Noah Williams"], ["14'", "goal", match.home, "Bàn thắng · Alex Martin", "Không có kiến tạo"]].map(([minute, type, team, title, detail]) => <div key={minute}><time>{minute}</time><i className={type}>{type === "goal" ? <Goal size={15} /> : <span />}</i><TeamMark name={team} size="sm" /><span><strong>{title}</strong><small>{detail}</small></span></div>)}</div></section><section className="content-card"><SectionHeading eyebrow="SO SÁNH" title="Thống kê trận đấu" />{[["Kiểm soát bóng", 56, 44, "%"], ["Sút", 14, 9, ""], ["Sút trúng đích", 6, 4, ""], ["Phạt góc", 7, 3, ""], ["xG", 1.82, 1.14, ""]].map(([label, home, away, suffix]) => <div className="stat-line" key={label}><strong>{home}{suffix}</strong><div><span>{label}</span><i><em style={{ width: `${Number(home) / (Number(home) + Number(away)) * 100}%` }} /></i></div><strong>{away}{suffix}</strong></div>)}</section></main><aside><div className="rail-card ai-preview"><Sparkles size={23} /><span className="eyebrow">SPORTPEEK AI</span><h3>{match.status === "finished" ? "Tóm tắt sau trận" : "Nhận định trước trận"}</h3><p>Hai đội tạo ra thế trận giàu cường độ với điểm nóng ở khu trung tuyến. Dữ liệu minh họa ghi nhận đội chủ nhà nhỉnh hơn về khả năng tạo cơ hội.</p><ul><li>Pressing quyết định nhịp trận</li><li>Bóng chết tạo khác biệt</li><li>Hai cánh có nhiều khoảng trống</li></ul></div><div className="rail-card"><SectionHeading eyebrow="TIN TỨC" title="Liên quan" />{news.slice(0, 3).map((item) => <NewsListItem item={item} key={item.id} />)}</div></aside></div></div>;
}
...
function LegalPage({ type }: { type: string }) {
  ...
  return <div className="legal-page"><span className="eyebrow">SPORTPEEK · PHÁP LÝ</span><h1>{titles[type] ?? "Thông tin"}</h1><p className="legal-lead">Cập nhật lần cuối: 13/07/2026. Nội dung dưới đây dành cho phiên bản minh họa của SportPeek.</p><section><h2>Nền tảng tổng hợp</h2><p>SportPeek là nền tảng tổng hợp tin thể thao. Chúng tôi không sở hữu nội dung bài viết gốc và chỉ hiển thị tiêu đề, metadata, trích đoạn ngắn, bản tóm tắt do AI tạo cùng liên kết tới nguồn.</p></section><section><h2>Quyền của nguồn tin</h2><p>Người dùng nên truy cập nguồn gốc để đọc nội dung đầy đủ. SportPeek không vượt paywall, không crawl trái phép và không tải lại video highlights. Nội dung video chỉ được nhúng khi đến từ nguồn chính thức hoặc được cấp phép.</p></section><section><h2>Đánh giá tự động</h2><p>Điểm nóng và độ tin cậy là đánh giá tự động dựa trên tín hiệu dữ liệu, không phải bảo đảm tuyệt đối. Tin đồn luôn được phân biệt với thông tin đã xác nhận.</p></section><section><h2>Liên hệ</h2><p>Yêu cầu gỡ nội dung hoặc phản hồi về nguồn tin có thể gửi tới copyright@sportpeek.local trong môi trường demo.</p></section></div>;
}
...
export default function SportPeekApp({ route }: { route: string }) {
  ...
  else if (segments[0] === "news" && segments[1]) page = <NewsDetail slug={segments[1]} bookmarked={bookmarks.has(news.find((item) => item.slug === segments[1])?.id ?? "n1")} onBookmark={toggleBookmark} />;
  ...
  return <div className="app-shell"><AppSidebar route={route} open={menuOpen} onClose={() => setMenuOpen(false)} /><div className="app-column"><Header onMenu={() => setMenuOpen(true)} onSearch={() => setSearchOpen(true)} theme={theme} onTheme={() => setTheme(theme === "dark" ? "light" : "dark")} /><div className="demo-bar"><ShieldCheck size={14} />Toàn bộ nội dung trên phiên bản này là dữ liệu minh họa, không phải tin tức thời gian thực.</div><div className="content-wrap">{page}</div><AppFooter /></div><MobileNavigation route={route} /><SearchCommand open={searchOpen} onClose={() => setSearchOpen(false)} /></div>;
}

...
export default function SportPeekApp({ route }: { route: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setRuntimeData({ newsItems: newsResponse?.data?.length ? newsResponse.data : news, matchItems: mergedMatches.length ? mergedMatches : matches, standingRows: tableResponse?.data?.length ? tableResponse.data : standings, newsReal: newsResponse?.demo === false, sportsReal: sportsResponses.some((result) => result.provider === "api-football" && result.demo === false), newsSources: newsResponse?.sources ?? [], aiTranslation: newsResponse?.aiTranslation === true });
    };
    void load();
    return () => { active = false; };
  }, []);

...
export function TeamMark({ name, size = "md" }: { name: string; size?: "sm" | "md" | "lg" }) {
...
export function SectionHeading({ eyebrow, title, action, href = "/news" }: { eyebrow?: string; title: string; action?: string; href?: string }) {
...
export function DataLoadingState({ label = "Đang tải dữ liệu thật" }: { label?: string }) {
...
export function EmptyState({ title, description }: { title: string; description: string }) {
  return <div className="empty-state"><Search size={28} /><strong>{title}</strong><p>{description}</p></div>;
}

export function ContentNotFound({ title, description }: { title: string; description: string }) {
...
export function NewsVisual({ item, compact = false, priority = false }: { item: NewsItem; compact?: boolean; priority?: boolean }) {
...
export function FormField({ label, value, disabled, name, type = "text", required = false }: { label: string; value: string; disabled?: boolean; name?: string; type?: string; required?: boolean }) {
  return <label className="form-field"><span>{label}</span><input name={name} type={type} defaultValue={value} disabled={disabled} required={required} /></label>;
}

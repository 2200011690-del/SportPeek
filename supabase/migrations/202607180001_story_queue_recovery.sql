-- Make story processing safe for overlapping Cloudflare cron invocations.
-- A raw article is leased to exactly one ingestion job at a time; expired
-- leases are recoverable and partially-persisted story links are authoritative.

alter table public.raw_articles
  add column if not exists processing_job_id uuid references public.ingestion_jobs(id) on delete set null,
...
create index if not exists raw_articles_story_queue_idx
  on public.raw_articles(processing_status, published_at, id)
  where processing_status in ('pending', 'failed');

create index if not exists raw_articles_story_lease_idx
  on public.raw_articles(processing_lease_expires_at)
  where processing_status = 'processing';

create index if not exists raw_articles_processing_job_idx
  on public.raw_articles(processing_job_id)
...
begin
  ...
  with released as (
    ...
    set
      ...
      processing_lease_expires_at = null,
      processing_error = 'Previous story-processing lease expired; queued again.'
    from public.news_sources as source
    where source.id = article.source_id
...
-- Repair the production backlog immediately when this migration is applied;
-- later cron claims repeat the same idempotent recovery step.

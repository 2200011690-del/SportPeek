-- Ranked, typo-tolerant archive search.
-- Keeps the existing trigram index useful while adding PostgreSQL full-text
-- ranking, freshness and editorial quality signals in one bounded RPC.

create or replace function public.normalize_news_search_text(value text)
returns text
language sql
immutable
parallel safe
  ...
  candidates as (
    ...
    where input.query_text <> ''
      and (
        ...
        or public.word_similarity(input.query_text, coalesce(story.search_text, '')) >= 0.18
      )
...
grant execute on function public.search_story_clusters(
  text, integer, integer, text, text, timestamptz, timestamptz, text, text, integer, text
) to service_role;
